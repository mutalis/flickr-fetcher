...
def get_random_keywords(number_of_words)
...
def setup
  ...
  tags << get_random_keywords(10 - tags.size) if tags.size < 10
  tags.flatten!
...
tags.each do |tag|
  response = fetch_image(tag)
  if response.nil?
    puts "Was not possible to fetch the image for the keyword: #{tag}"
  else
    save_image(response.parsed_response, tag)
    crop_image(tag)
  end
end
